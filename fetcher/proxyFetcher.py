...
class ProxyFetcher(object):
    ...
    def freeProxy06():
        ...
        while has_next:
            sleep(1)
            url = f"http://www.binglx.cn/?page={page_num}"
            try:
                tree = WebRequest().get(url).tree
                proxy_list = tree.xpath(".//table//tr")
                for tr in proxy_list[1:]:
                    yield ":".join(tr.xpath("./td/text()")[0:2])
                target_text = "下一页"
                elements = tree.xpath("//a[contains(text(),'%s')]" % target_text)
                has_next = bool(elements)
                page_num += 1
    ...
    def freeProxy07():
        ...
        for url in url_list:
            tree = WebRequest().get(url).tree
            page_count_element = tree.xpath("//div[@id='listnav']/ul/strong/font")
            if page_count_element:
                page_count_text = page_count_element[0].tail
                match = re.findall("/(\d+)", page_count_text)
                if match:
                    page_count = int(match[0])
                else:
                    page_count = 1
            for i in range(1, page_count + 1):
                params = {"page": i}
                new_url = f"{url}&page={params['page']}"
                tree = WebRequest().get(new_url).tree
                proxy_list = tree.xpath(".//table//tr")
                sleep(1)  # 必须sleep 不然第二条请求不到数据
                for tr in proxy_list[1:]:
                    yield ":".join(tr.xpath("./td/text()")[0:2])
    ...
    def freeProxy10():
        ...
        while has_next:
            sleep(1)
            r = WebRequest().get(
                f"https://www.89ip.cn/index_{page_num}.html", timeout=10
            )
            proxies = re.findall(
    ...
    def freeProxy12():
        ...
        while has_next:
            sleep(1)
            url = f"http://www.69ip.cn/?page={page_num}"
            try:
                tree = WebRequest().get(url).tree
                proxy_list = tree.xpath(".//table//tr")
                for tr in proxy_list[1:]:
